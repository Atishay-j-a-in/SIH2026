...
def compute_instance_losses(combined_loss, label_map, preset_instance_threshold, iteration, image_name, args):
    ...
    heatmap_norm = heatmap_norm.to(device="cuda", dtype=torch.float32)
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args, progress_callback=None):
    first_iter = 0
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if args.use_masks and iteration > args.mask_start_iter and heatmap_binary is not None:
        ...
        with torch.no_grad():
            ...
            if progress_callback and (iteration == first_iter or iteration % 100 == 0):
                progress_callback(
                    "train",
                    int(5 + 90 * iteration / opt.iterations),
                    f"Training iteration {iteration}/{opt.iterations}",
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sam2_cfg", type=str, default="configs/sam2/sam2_hiera_l.yaml")
