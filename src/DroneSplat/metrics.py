...
def calculate_metrics(img1_path, img2_path, loss_fn):
    rendered_image = Image.open(img1_path).convert("RGB")
    ground_truth_image = Image.open(img2_path).convert("RGB")
    if ground_truth_image.size != rendered_image.size:
        ground_truth_image = ground_truth_image.resize(rendered_image.size, Image.Resampling.LANCZOS)

    img1 = np.array(rendered_image)
    img2 = np.array(ground_truth_image)
